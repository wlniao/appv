...
const wln = { version: '2.4.6', InBrowser: window && window.navigator && window.navigator.userAgent, OS: '' }
if (wln.InBrowser) {
	let _ua = window.navigator.userAgent.toLowerCase()
  if (_ua.indexOf('wlnapp') >= 0) {
  	wln.OS = 'app'
  } else if (_ua.indexOf('miniprogram') >= 0) {
    wln.OS = 'xcx'
...
wln.redirectTo = function (obj) {
  let url = typeof (obj) == 'string' ? obj : obj.url
  if (this.InBrowser) {
    wln.router.replace(url)
  } else {
    uni.redirectTo({url: url})
  }
}

wln.getLocation = function(successFn, errorFn) {
	if (wln.OS == 'wx' || wln.OS == 'xcx') {
		try{
			wx.getLocation({ type: 'wgs84', success: function (res) {
				successFn({latitude: res.latitude, longitude: res.longitude})
			}})
		} catch(e) {
			errorFn(e)
		}
